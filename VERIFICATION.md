...
| A screenshot of the compliance panel | The coupon-code pattern's digit lookahead scanned the whole message rather than the token, so the company's own name `GIMME` was reported as an unverified coupon code in every message containing a digit — blocking its own campaigns | Lookahead bounded to the token |
| Live acceptance walk-through | Every Smart Reorder reminder cancelled itself as ALREADY_ORDERED, because the send-time window was worked back as *predicted − interval* and caught the customer's own last order | The exact anchor is stored on the message when it is written |
| Live acceptance walk-through | The dry run reported 0 customers on a campaign with 47, because it walked enrollments — which do not exist until activation, the thing the dry run exists to decide | The builder resolves the audience directly |
| Uploading a full template row | `typical_order_weekday` and `typical_order_hour` were counted off naive UTC while the minute beside them came from the local-time engine, so one time was assembled from two clocks — and the same wrong day fed the LLM prompt context | Counted off `to_local(ordered_at)`, like everything else that reasons about the customer's clock |
